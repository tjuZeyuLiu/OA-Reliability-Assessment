...
tic;
[lc0,spnum0,sp]=sp_ld17_cal_init(mpc0,ldlv,datab,totsp,[]);
...
for i=1:size(CtgBr{1},1)
        ...
        if  i == 1
            [lc{CtgBr{1}(i,1)}(CtgBr{1}(i,2)),spnum1(i),sp]=sp_ld17_cal(mpc,ldlv,datab,sp,spnum0,totsp,CtgCpntList(CtgBrList, 2));
        else
           [lc{CtgBr{1}(i,1)}(CtgBr{1}(i,2)),spnum1(i),sp]=sp_ld17_cal(mpc,ldlv,datab,sp,spnum1(i-1),totsp,CtgCpntList(CtgBrList, 2));
        end
...
for i= 1:1:size(CtgBr{2},1)
    ...
    mpc.branch(CtgCpntList(CtgBrList, 2),[3,4]) = 0;
    if i == 1
           [lc{CtgBr{2}(i,1)}(CtgBr{2}(i,2)),spnum2(i),sp]=sp_ld17_cal(mpc,ldlv,datab,sp,spnum1(size(CtgBr{1},1)),totsp,CtgCpntList(CtgBrList, 2));
    else
           [lc{CtgBr{2}(i,1)}(CtgBr{2}(i,2)),spnum2(i),sp]=sp_ld17_cal(mpc,ldlv,datab,sp,spnum2(i-1),totsp,CtgCpntList(CtgBrList, 2));
    end
   if (mod(i,30000)==0)
       disp(i);
   end
...
for i= 1:size(CtgBr{3},1)
       ...
       mpc.branch(CtgCpntList(CtgBrList, 2), [3,4]) = 0;
     if i == 1
           [lc{CtgBr{3}(i,1)}(CtgBr{3}(i,2)),spnum3(i),sp]=sp_ld17_cal(mpc,ldlv,datab,sp,spnum2(size(CtgBr{2},1)),totsp,CtgCpntList(CtgBrList, 2));
    else
           [lc{CtgBr{3}(i,1)}(CtgBr{3}(i,2)),spnum3(i),sp]=sp_ld17_cal(mpc,ldlv,datab,sp,spnum3(i-1),totsp,CtgCpntList(CtgBrList, 2));
    end
   if (mod(i,30000)==0)
       disp(i);
   end
...
toc
savestr=strcat('sp_ld17_cs',casenum,'_lv',ldlvnum,'20220906','.mat');
